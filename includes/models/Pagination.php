...
class Pagination {
    ...
    public function paginacao($range = 2) {
        ...
        $pagination = "<div style='text-align:center;' class='btn-toolbar' role='toolbar'>\n";
        $pagination .= "<div class='btn-group'>";
        if (1 != $this->sets) {

            if ($this->pg > 1) {
                $pagination .= "<a class='btn btn-default' href='" . $this->self_url. -- $current . "'><i class='icon-chevron-left'></i>-</a>";
            }
            if ($this->pg > 10) {
            ...
            for ($i = 1; $i <= $this->sets; $i++) {
                if (1 != $this->sets && (!($i >= $this->pg + $range + 1 || $i <= $this->pg - $range - 1) || $this->sets <= $showitems )) {
                    $pagination .= ($this->pg == $i) ? "<a class='btn btn-default' href='" . $this->self_url . $this->pg . "' class='colored'>" . $i . "</a>" : "<a class='btn btn-default' href='" . $this->self_url . $i . "'>" . $i . "</a>";
                }
            }
            ...
            if ($this->pg != $this->sets) {
                $pagination .= "<a class='btn btn-default' href='" . $this->self_url . ($this->pg + 1) . "'>-<i class='icon-chevron-right'></i></a>";
            }
        }
